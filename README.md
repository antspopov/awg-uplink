...
- **Файрвол панели** — через **UFW** (переключатель «включить», по умолчанию включён): allowlist TCP на egress/ingress и запрет входа на `awg-uplink` добавляются **только** с меткой `awg-web-ui-fw` — вручную добавленные в UFW порты скрипт не трогает. Нужны **`sudo ufw enable`** и пакет **`ufw`**. **Georouting и DNS-transport-lock** по-прежнему на nftables.
