...
```bash
cd /path/to/awg-uplink
sudo git pull origin main   # или нужная ветка
...
- **Файрвол панели** — по умолчанию **nftables** (отдельная таблица `inet awg_webui_fw`). Можно выбрать **UFW**: те же allowlist портов и запрет входа на `awg-uplink` оформляются правилами с меткой `awg-web-ui-fw` (удобно смотреть `ufw status numbered`). Нужны пакет **`ufw`**, **`sudo ufw enable`**, и осознанность к порядку с остальными правилами UFW. **Georouting и DNS-transport-lock** по-прежнему на nftables — UFW их не заменяет.
